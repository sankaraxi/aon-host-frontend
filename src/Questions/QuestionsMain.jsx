...
export default function QuestionsMain() {
    ...
    //     try {
    //         const response = await fetch('http://103.118.158.33/aon-api/api/run-script', {
    //             method: 'POST',
    //             headers: {
    ...
    //     try {
    //       const res = await fetch('http://103.118.158.33/aon-api/api/run-script', { method: 'POST' });
      ...
      const handleStartAssessmentII = async (selectedFramework) => {
        ...
        const interval = setInterval(() => {
            ...
            if (countdown === 0) {
                ...
                window.location.href = `/workspace/${id}/${selectedFramework}/${userQuestion}/${dPort}/${oPort}`;
            }
        }, 1000);

        try {
            const res = await fetch('http://103.118.158.33/aon-api/api/run-script', 
                {
                    method: 'POST',
                    headers: {
                      'Content-Type': 'application/json',
